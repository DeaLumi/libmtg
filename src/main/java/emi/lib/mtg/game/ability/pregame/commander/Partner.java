...
public interface Partner extends CommandZoneOverride {
	...
	abstract class PartnerGroup implements Partner {
		...
		@Override
		public Format.Validator.Result check(Card.Print source, Deck deck, Collection<? extends Card.Print> commanders) {
			...
			Card.Print partner = commanders.stream().filter(pr -> pr != source).reduce(null, (a, b) -> (a != null) ^ (b != null) ? (a != null ? a : b) : null);
