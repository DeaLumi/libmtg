...
public interface ImageSource {
	...
	/**
	 * Attempt to open an an image of the given card print. Ideally, this means, "show me what I'd see if I opened a
	 ...
	 *
	 * @param print The print of the card of which to find an image.
	 ...
	 */
	BufferedImage open(Card.Print print) throws IOException;
	 ...
	 *
	 * @param facePrint The print of the face for which to find an image.
	 * @return An input stream to an image of that face, or null if this source couldn't find such an image.
	 * @throws IOException if an error occurs while opening a card image the source located.
	 */
	BufferedImage open(Card.Print.Face facePrint) throws IOException;
	...
	default BufferedImage open(Card card) throws IOException {
		Iterator<? extends Card.Print> prints = card.prints().iterator();

		if (prints.hasNext()) {
			return open(prints.next());
		} else {
			return null;
		}
	}
	...
	default BufferedImage open(Card card, Card.Face face) throws IOException {
		...
		if (prints.hasNext()) {
			Iterator<? extends Card.Print.Face> printedFaces = prints.next().faces(face).iterator();
